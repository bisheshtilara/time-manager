<template>
  ...
  <div v-else class="border rounded-lg shadow-md">
    ...
    <div v-else class="flex justify-between px-2 w-full items-center">
      <p class="text-xs font-medium">Average Working Times</p>
...
const getAllTeams = async () => {
  ...
  getTeamWorkingTime(allTeams.value[0].id);
};
...
const getTeamWorkingTime = async (id) => {
  ...
  for (const user of data.users) {
    ...
    if (workingTimesResponse.data.length === 0) {
      ...
      const totalWorkingTime = workingTimesResponse.data.reduce(
        (prev, current) => prev + current.workingTime,
        0
      );
      const averageWorkingTime =
        totalWorkingTime / workingTimesResponse.data.length;
      labels.value.push(userInfo.username);
      dataset.value.push(averageWorkingTime.toFixed(0));
    }
  }
